...
val_set = [
    dataset[int(0.2 * i * len(dataset)) : int(0.2 * (i + 1) * len(dataset))]
    for i in range(5)
]

# Blackbox objective
def test_model(model: ContextAE(5, 2), batch: int):
    ...
    for val in val_set:
        loader = th.utils.data.DataLoader(dataset=val, batch_size=batch, shuffle=False)
...
with open(
    os.path.join(os.getcwd(), "tmp/AE_stuff/AE_Tune_WD_norm_1", "configs.json"), "r"
) as f:
...
for i in range(9):
    ...
    print(
        f"Batch_Size: {batch}    LR: {rate}    Decay: {decay}    Val_losses: {test_model(model, 1)}"
    )
